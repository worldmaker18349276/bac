...
removeMorphism (src, tgt) bac = do
  ...
  Monad.join $ forUnder src bac $ \curr -> \case
    ToBoundary -> do
      let filtered_edges =
...
prepareForAddingMorphism src tgt src_alts tgt_alts val bac = do
  ...
  sequence_ $ forUnder src bac $ \curr -> \case
    ToBoundary -> Just []
    ToInner results -> do
      results' <- traverse sequence results
      let pairs = do
...
insertMorphism (src, tgt) (val1, val2) bac = do
  ...
  forUnder src bac $ \curr -> \case
    ...
    ToInner results -> forEdges curr results $ \edge res -> case toMaybe res of
      Nothing -> [edge]
...
mergeMorphismsAggressively src tgts bac = do
  ...
  fmap fst $ Monad.join $ forUnder src bac $ \curr -> \case
    ToBoundary -> do
